...
function authenticate ()
{
	...
	if (!isset ($user_auth_src) or !isset ($require_local_account))
		throw new Exception ('secret.php: either user_auth_src or require_local_account are missing', RackTablesError::MISCONFIGURED);
	if (isset ($_REQUEST['logout']))
		throw new Exception ('', RackTablesError::NOT_AUTHENTICATED); // Reset browser credentials cache.
	switch ($user_auth_src)
	{
		...
		case 'ldap':
			...
			(
				...
				throw new Exception ('', RackTablesError::NOT_AUTHENTICATED);
		...
		case 'httpd':
			...
			(
				...
				throw new Exception ('The web-server didn\'t authenticate the user, although ought to do.', RackTablesError::MISCONFIGURED);
			$remote_username = $_SERVER['REMOTE_USER'];
			break;
		default:
			throw new Exception ('Invalid authentication source!', RackTablesError::MISCONFIGURED);
			die;
	}
...
function queryLDAPServer ($username, $password)
{
	...
	)
	{
		...
		throw new Exception ('LDAP misconfiguration. Cannon build username for authentication.', RackTablesError::MISCONFIGURED);
