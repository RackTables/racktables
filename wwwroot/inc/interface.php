...
// records 100~199 with fatal error messages
				...
				110 => array ('code' => 'error', 'format' => 'failed adding a record'),
				111 => array ('code' => 'error', 'format' => 'failed deleting a record'),
...
function renderRackSpaceForObject ($object_id)
{
	...
	foreach ($workingRacksData as $rack_id => $rackData)
	{
		...
		echo "<th width='20%'><a href='javascript:;' onclick=\"toggleColumnOfAtoms('${rack_id}', '2', ${rackData['height']})\">Back</a></th></tr>\n";
		echo "</table>\n<br>\n";
		echo "<input type='button' onclick='uncheckAll();' value='Uncheck all'></center>\n";
		echo '</td>';
	}
	echo "</tr></table>";
	finishPortlet();
		...
		foreach ($addrspaceList as $netinfo)
		{
			...
			foreach (array ($netinfo['db_first'] => 'network', $netinfo['db_last'] => 'broadcast') as $ip => $comment)
				...
				(
					...
					! count ($netinfo['addrlist'][$ip]['rslist']) and
					! count ($netinfo['addrlist'][$ip]['allocs']) and
					! count ($netinfo['addrlist'][$ip]['lblist'])
				)
					$pure_auto++;
