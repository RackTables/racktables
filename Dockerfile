...
RUN service mysql start && mysql -u root -e "CREATE DATABASE racktables_db CHARACTER SET utf8 COLLATE utf8_general_ci;" && mysql -u root -e "CREATE USER racktables_user@localhost IDENTIFIED BY 'MY_SECRET_PASSWORD';" && mysql -u root -e "GRANT ALL PRIVILEGES ON racktables_db.* TO racktables_user@localhost;" && mysql -u root -e "flush PRIVILEGES;"
